...
target.adj <- target + 0.7
min.fe <- min(cntry.fe$fe) + 0.7
...
years <- data.frame(year = seq(2021, 2100, 1))
...
years2 <- data.frame(year = seq(2020, 2100, 1))
...
fe.convergence %>%
  filter(countrycode == "SOM") %>%
  ggplot(aes(x = year, y = fe, color = scenario)) +
  geom_line(size = 1) +
  labs(y = "Fixed effect", x = 'Year')

# Add the converged fixed effects to the projections & the constant fe

const.fe <- cntry.fe %>% filter(year == 2000) %>% select(-year) %>% rename(fec = fe)

projections <- projections.yr %>% 
  left_join(const.fe, by = c('countrycode')) %>% 
...
projections.ext <- projections %>%  
  mutate(gov = log(gdppc)*(c1$estimate) + post.secondary*(c2$estimate) + mys.gap*(c3$estimate) + fe,
         gov = ifelse(gov>1, 1, gov), 
         gov.const.fe = log(gdppc)*(c1$estimate) + post.secondary*(c2$estimate) + mys.gap*(c3$estimate) + fec,
         gov.const.fe = ifelse(gov.const.fe>1, 1, gov.const.fe) )
...
projections.ext <- projections.ext %>% 
  mutate(ID = paste(countrycode, year, scenario)) %>% 
  filter(!duplicated(ID)) %>% 
  filter(!countrycode == 'TLS') %>% 
  drop_na(gov)# Remove East Timor as there was no data on governance for it
