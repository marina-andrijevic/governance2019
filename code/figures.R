...
ggplot() +
  geom_point(dat = filter(results, year < 2015 & countrycode %in% sample), aes(x = year, y = governance, color = scenario), size = 2) +
  geom_line(dat = filter(results, year >= 2015 & countrycode %in% sample), aes(x = year, y = governance, color = scenario), size = 1.5) +
...
pres <- results %>% filter(year == 2015 & scenario == 'Observed') %>% select(governance)
